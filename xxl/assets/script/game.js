...
cc.Class({
    ...
    onUpdateHighScore() {
        ...
        httpRequest.open('GET', 'http://40.82.157.198:8083/getHighScore', true);//第二步：打开连接  将请求参数写在url中  ps:"./Ptest.php?name=test&nameone=testone"
    ...
    onGameHighScoreSave: function (data) {
        ...
        //
        let httpRequest = new XMLHttpRequest();//第一步：建立所需的对象
        httpRequest.open('GET', `http://40.82.157.198:8083/setHighScore?name=${name}&score=${highScore}`, true);//第二步：打开连接  将请求参数写在url中  ps:"./Ptest.php?name=test&nameone=testone"
        httpRequest.send();//第三步：发送请求  将请求参数写在URL中
        /**
         * 获取数据后的处理程序
         */
